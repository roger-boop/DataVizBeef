...
In order to determine whether or not SAHA and PMA have the same effect on the cells we will perform a pca and see how close the clusters of each condition are. 

```{r}
Y_SAHA_PMA <- X[((treatments$conditions == "SAHA") == TRUE) | ((treatments$conditions == "PMA")== TRUE),]

treatments$totalExpr <- rowSums(Y)

treat_SAHA_PMA <- treatments[(treatments$conditions == "SAHA") | (treatments$conditions == "PMA"),]

Y_SAHA_PMA <- Y_SAHA_PMA[, colSums(Y_SAHA_PMA)>0]
Y_SAHA_PMA = Y_SAHA_PMA/rowSums(Y_SAHA_PMA)
...
pl <- ggplot(components, aes(x = PC1, y = PC2, color = totalExpr, shape = conditions)) + geom_point() + scale_colour_gradient(low = "#00FF00", high = "#FF0000") + scale_shape_manual(values = c(3,17, 11)) + theme_light() + labs(color = "Expression", shape = "Conditions", title = "PCA")
...
In order to find three genes that are activated by SAHA but not by PMA, we will subset all cells with SAHA and PMA respectively and sum all the expressions of their genes. This is not the total expression of the cell, but the total expression of each gene. Then, we take the 3 most expressed genes in SAHA and the 3 most expressed genes in PMA, and compare them.

```{r}
SAHA_cells <- treatments_no_outlier %>% filter(conditions == "SAHA") %>% pull(cellnames)
PMA_cells <- treatments_no_outlier %>% filter(conditions == "PMA") %>% pull(cellnames)
SAHA_Y <- Y[SAHA_cells,]
PMA_Y <- Y[PMA_cells,]
